...
class MultiNet(nn.Module):
    def __init__(self, backbone='efficientnet-b5', drop=0.4, num_classes=54):
        ...
        self.last_linear = nn.Sequential(
            nn.Conv2d(in_channels=self.img_model._blocks[2]._bn2.num_features +
                                  self.img_model._blocks[7]._bn2.num_features +
                                  self.img_model._blocks[12]._bn2.num_features +
                                  self.img_model._blocks[16]._bn2.num_features +
                                  self.img_model._blocks[26]._bn2.num_features +
                                  self.img_model._blocks[35]._bn2.num_features,
                      out_channels=256, kernel_size=5, stride=2, padding=0, dilation=1, bias=False),
            nn.BatchNorm2d(256),
    ...
    def upsample(self, x,  scale_factor):
        out = F.interpolate(x, scale_factor=scale_factor, mode='bilinear', align_corners=False)
        return out

    def attention(self, x):
        # attention
        conv = nn.Conv2d(x.size(1), 1, 1, 1, 0, 1, bias=False).cuda()
        bn = nn.BatchNorm2d(1).cuda()
        x = bn(conv(x))
    ...
    def forward(self, x_img):
        ...
        for idx, block in enumerate(self.img_model._blocks):
            ...
            # if idx == 0:
            #     alpha = self.attention(x_img)
            # else:
            #     torch.cat((alpha, self.attention(x_img), 0)


            if idx == 2:
                feature_2 = F.adaptive_avg_pool2d(x_img, 14)
            if idx == 7:
                feature_7 = F.adaptive_avg_pool2d(x_img, 14)
            if idx == 12:
                feature_12 = F.adaptive_avg_pool2d(x_img, 14)
            if idx == 16:
                feature_16 = F.adaptive_avg_pool2d(x_img, 14)
            # if idx == 21:
            #     feature_21 = F.adaptive_avg_pool2d(x_img, 14)
            # if idx == 22:
            #     feature_22 = F.adaptive_avg_pool2d(x_img, 14)
            # if idx == 23:
            #     feature_23 = F.adaptive_avg_pool2d(x_img, 14)
            if idx == 26:
                feature_26 = F.adaptive_avg_pool2d(x_img, 14)
            if idx == 35:
                feature_35 = F.adaptive_avg_pool2d(x_img, 14)
        ...
        # concat
        x_cat = torch.cat((feature_2,feature_7,feature_12,feature_16,
                           feature_26,feature_35),dim=1)
