...
class MultiNet(nn.Module):
    def __init__(self, backbone='efficientnet-b5', drop=0.4, num_classes=54):
        ...
        self.last_linear = nn.Sequential(
            nn.Conv2d(in_channels=self.img_model._blocks[2]._bn2.num_features +
                                  ...
                                  self.img_model._blocks[13]._bn2.num_features +
                                  self.img_model._blocks[14]._bn2.num_features +
                                  self.img_model._blocks[18]._bn2.num_features +
                                  self.img_model._blocks[24]._bn2.num_features +
                                  self.img_model._blocks[25]._bn2.num_features +
                                  self.img_model._blocks[27]._bn2.num_features +
                                  self.img_model._blocks[33]._bn2.num_features,
                      out_channels=432, kernel_size=5, stride=2, padding=0, dilation=1, bias=False),
            nn.BatchNorm2d(432),
            nn.LeakyReLU(inplace=True),
            # nn.Conv2d(512, 256, 1, 1, 0, 1, bias=False),
            # nn.BatchNorm2d(256),
            # nn.LeakyReLU(inplace=True),
    ...
    def forward(self, x_img):
        ...
        for idx, block in enumerate(self.img_model._blocks):
            ...
            if idx == 2:
                feature_2 = F.adaptive_avg_pool2d(x_img, 28)
            if idx == 7:
                feature_7 = F.adaptive_avg_pool2d(x_img, 28)
            if idx == 13:
                feature_13 = x_img
            if idx == 14:
                feature_14 = x_img
            ...
            if idx == 25:
                feature_25 = x_img
            if idx == 27:
                feature_27 = self.upsample(x_img, scale_factor=2)
            if idx == 33:
                feature_33 = self.upsample(x_img, scale_factor=2)
        ...
        # concat
        x_cat = torch.cat((feature_2,feature_7,feature_13,feature_14,feature_18,
                           feature_24,feature_25,feature_27,feature_33),dim=1)
